...
def convert_ckpt(root_dir, features, net_serializer):
    ...
    for ckpt in ckpts:
        ...
        if not os.path.exists(nnue_file_name):
            command = "{} {} {} {} --features={}".format(sys.executable, net_serializer, ckpt, nnue_file_name, features)
...
def run_match(best, root_dir, c_chess_exe, concurrency, book_file_name, stockfish_base, stockfish_test, tc, hash_size):
    ...
    command = "{} -each {} option.Hash={} option.Threads=1 -gauntlet -games 200 -rounds 1 -concurrency {}".format(
        c_chess_exe, tc, hash_size, concurrency
    )
    command = (
        command
...
def run_round(
    ...
    convert_ckpt(root_dir, features, net_serializer)
    ...
    run_match(best, root_dir, c_chess_exe, concurrency, book_file_name, stockfish_base, stockfish_test, tc, hash_size)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="tc=4+0.4",
        help="Time control in c-chess-cli syntax. For example `tc=1000+1 nodes=10000`."
    )
    parser.add_argument(
        "--hash_size",
        type=int,
        default=8,
        help="Hash size in MiB."
    )
    parser.add_argument(
        "--features",
        type=str,
        default="HalfKP^",
        help="The feature set of the .ckpt files. Used for serialization."
    )
    parser.add_argument(
        "--net_serializer",
        type=str,
        default="serialize.py",
        help="Path to the serialize.py."
    )
