...
Training a network is really just minimizing a loss function, which needs to be smooth and has a minimum at "optimal" evaluation (the training target). For the purpose of NNUE this is done by gradient descent through usual machine learning methods. There are two (three) popular loss functions for training NNUE.
...
This loss function is a small modification of MSE. It changes from a quadratic function to a linear one at a certain threshold, effectively capping the gradient at some constant value.

```
predicted = model(...)

a = 5 # This parameter controls when the transition from quadratic to linear happens
loss = min((predicted - target)**2, a * abs(predicted - target))
```

##### loss

![](img/huber_loss.png)
![](img/huber_loss_contour.png)

##### grad

![](img/huber_loss_grad.png)
![](img/huber_loss_grad_contour.png)
...
predicted = sigmoid(model(...) / scaling)
target = sigmoid(target / scaling)

# The first term in the loss has 0 gradient, because we always
# differentiate with respect to `predicted`, but it makes the loss nice
# in the sense that 0 is the minimum.
loss =  (target * log(target)    + (1 - target) * log(1 - target))
       -(target * log(predicted) + (1 - target) * log(1 - predicted))
```
...
Obviously the MSE and Huber loss can also work in WDL space, but cross entropy is the more natural choice for that.
