        ...
        for (;;)
        {
            if (!m_stream->hasNext())
            {
                if (m_cyclic)
                {
                    ...
                    m_stream = std::make_unique<binpack::CompressedTrainingDataEntryReader>(
                      m_filename, openmode);
                    reopenedFileOnce = true;
...
open_sfen_input_file(const std::string&                            filename,
                     bool                                          cyclic,
...
inline std::unique_ptr<BasicSfenInputStream> open_sfen_input_file_parallel(
  ...
  std::function<bool(const TrainingDataEntry&)> skipPredicate = nullptr) {
    ...
    if (has_extension(filenames[0], BinSfenInputStream::extension))
        ...
        return std::make_unique<BinpackSfenInputParallelStream>(concurrency, filenames, cyclic,
