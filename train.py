...
def data_loader_cc(train_filename, val_filename, num_workers, batch_size, filtered, main_device):
  # Epoch and validation sizes are arbitrary
  epoch_size = 100000000
  val_size = 1000000
  train_infinite = nnue_dataset.SparseBatchDataset(halfkp.FACTOR_NAME, train_filename, batch_size, num_workers=num_workers, filtered=filtered, device=main_device)
  val_infinite = nnue_dataset.SparseBatchDataset(halfkp.FACTOR_NAME, val_filename, batch_size, filtered=filtered, device=main_device)
...
def main():
  ...
  if args.py_data:
    ...
    train, val = data_loader_cc(args.train, args.val, args.num_workers, batch_size, args.smart_fen_skipping, main_device)
