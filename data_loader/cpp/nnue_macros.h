 ...
 */
#if defined(_MSC_VER)
    #define NNUE_EXPORT __declspec(dllNNUE_EXPORT)
 ...
 * The NNUE_CDECL calling convention is only applicable to 32-bit x86 architectures.
 * 64-bit x86 (x86_64) and ARM (aarch64, etc.) use unified calling conventions,
 * so explicitly defining NNUE_CDECL there triggers compiler warnings or errors.
 */
#if defined(_M_IX86) || defined(__i386__)
    #if defined(_MSC_VER)
        #define NNUE_CDECL __NNUE_CDECL
    #else
        #define NNUE_CDECL __attribute__((__NNUE_CDECL__))
    #endif
#else
    #define NNUE_CDECL
#endif
 ...
 */
#define NNUE_ABI NNUE_EXTERN_C NNUE_EXPORT
